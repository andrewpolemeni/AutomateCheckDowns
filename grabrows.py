...
def GetStudentData(query_filename):
    ...
    df1.drop(df2.loc[df1['Grade']=='F'].index, inplace=True) # Delete grades that are equal to F
    df1.drop(df2.loc[df1['Grade']=='FN'].index, inplace=True) # Delete grades that are equal to FN
    df1.drop(df2.loc[df1['Grade']=='W'].index, inplace=True) # Delete grades that are equal to W
