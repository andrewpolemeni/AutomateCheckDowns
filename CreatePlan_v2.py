...
def CreateStudentFile(df):
    ...
    ws2.cell(row=5, column=3).value = str(df.loc[j]["First Name"]) + " " + str(df.loc[j]["Last"])
    ws2.cell(row=6, column=3).value = str(df.loc[j]["ID"])
...
df1 = GetStudentData("Query.xlsx") #CREATE A VARIABLE FOR THE QUERY SHEET
ids = df1['ID'].unique().tolist() # Creates a list of the ids
GenerateSingleSpreadsheet(df1, ids[0])
